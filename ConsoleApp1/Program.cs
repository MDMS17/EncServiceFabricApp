...
namespace ConsoleApp1
{
    class Program
    {
        ...
        {
            List<Tuple<string, string>> testExcept = new List<Tuple<string, string>>();
            testExcept.Add(Tuple.Create("testfile1.txt", "20"));
            testExcept.Add(Tuple.Create("testfile2.txt","19"));
            testExcept.Add(Tuple.Create("testfile3.txt", "18"));
            List<Tuple<string, string>> testExcept2 = new List<Tuple<string, string>>();
            Tuple<string, string>[] testExcept3 = new Tuple<string, string>[10];
            testExcept.CopyTo(testExcept3);
            testExcept2 = testExcept3.ToList();
            testExcept2.RemoveAt(2);
            List<Tuple<string, string>> result = testExcept.Except(testExcept2).ToList();
            Common.StatusCodes = "'TBP','ENC','CLS'";
            Common.ClaimType = "HCF";
            Common.GroupNumbers = "'810','812'";
            Common.StartDate = "05/05/2021";
            Common.EndDate = "05/12/2021";
            Common common = new Common();
            string ssss = common.QueryMeditracCounts;
            string testStr = null;
            string testStr2 = testStr??"".PadRight(4, ' ');
            List<string> counties = new List<string> { "3203304305", "3203304306" };
            counties = counties.Select(x => x.Substring(x.Length -3)).Distinct().ToList();
            string s = "";
            //byte[] data = new byte[] {0x14,0x11,0x00,0x00,0x00,0x00,0x64,0x70,0xee,0x2b,0x69,0x6d,0x43,0xa0,0x0a,0x00 };
            //IConfigurationRoot configuration = new ConfigurationBuilder().SetBasePath(Directory.GetCurrentDirectory()).AddJsonFile("appsettings.json").Build();
            //string sourcePath = configuration["Source_CMS277CA"];
            //SubcacheContext context = new SubcacheContext();
            //int pages = context.ClaimHeaders.Count(x => x.ExportType == "PDHCS" + CountyCode) / 5000;
            ...
            //    SegmentCount = 0;
            //    HLID = 1;
            //    List<ClaimHeader> headers = await context.ClaimHeaders.Where(x => x.ExportType == "PDHCS" + CountyCode).OrderBy(x => x.ID).Skip(i * 5000).Take(5000).ToListAsync();
            //    if (headers.Count > 0)
            //    {
