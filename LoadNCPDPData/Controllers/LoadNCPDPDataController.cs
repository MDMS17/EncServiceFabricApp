...
namespace LoadNCPDPData.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class LoadNCPDPDataController : ControllerBase
    {
        private readonly ILogger<LoadNCPDPDataController> _logger;
        private readonly NCPDPContext _context;
        public LoadNCPDPDataController(ILogger<LoadNCPDPDataController> logger, NCPDPContext context)
        {
            _logger = logger;
            _context = context;
        }
        //LoadNCPDPData
        [HttpGet]
        public List<string> GetNewNCPDPDataFiles()
        {
            _logger.Log(LogLevel.Information, "inquiry unprocessed files");
            IConfigurationRoot configuration = new ConfigurationBuilder().SetBasePath(Directory.GetCurrentDirectory()).AddJsonFile("appsettings.json").Build();
            string sourcePath = configuration["Source_NCPDP"];
            string archivePath = configuration["Archive_NCPDP"];
        ...
        //LoadNCPDPData/1
        [HttpGet("{id}")]
        public List<string> ProcessNCPDPDataFiles(long id)
        {
            List<string> result = new List<string>();
            _logger.Log(LogLevel.Information, "process new NCPDP flat files");
            IConfigurationRoot configuration = new ConfigurationBuilder().SetBasePath(Directory.GetCurrentDirectory()).AddJsonFile("appsettings.json").Build();
            string sourcePath = configuration["Source_NCPDP"];
            string archivePath = configuration["Archive_NCPDP"];
            ...
            foreach (FileInfo fi in fis)
            {
                ...
                s = null;
                Match headerMatch = oRxParse_Header.Match(lines[0]);
                ...
                _context.Headers.Add(fh);
                _context.SaveChanges();
                int detailId = 0;
                for (int i = 1; i < lines.Length; i++) 
                {
                    if (string.IsNullOrEmpty(lines[i])) continue;
                    Match detailMatch = oRxParse_Detail.Match(lines[i]);
                    if (detailMatch.Success && lines[i].StartsWith("DE"))
                    {
                        ...
                        _context.Details.Add(hd);
                        _context.SaveChanges();
                        detailId = hd.DetailID;
                        continue;
                    }
                    Match compound1Match = oRxParse_Compound1.Match(lines[i]);
                    if (compound1Match.Success && lines[i].StartsWith("CD1"))
                    {
                        ...
                        _context.Compound1s.Add(hc1);
                        _context.SaveChanges();
                        continue;
                    }
                    Match compound2Match = oRxParse_Compound2.Match(lines[i]);
                    if (compound2Match.Success && lines[i].StartsWith("CD2"))
                    {
                        ...
                        _context.Compound2s.Add(hc2);
                        _context.SaveChanges();
                        continue;
                    }
                    Match trailMatch = oRxParse_Trailer.Match(lines[i]);
                    if (trailMatch.Success && lines[i].StartsWith("PT"))
                    {
                        fh.Total_Record_Count = trailMatch.Groups["Total_Record_Count"].Value.Trim();
                        fh.Total_Net_Amount_Due = trailMatch.Groups["Total_Net_Amount_Due"].Value.Trim();
                        fh.Total_Gross_Amount_Due = trailMatch.Groups["Total_Gross_Amount_Due"].Value.Trim();
                        fh.Total_Patient_Pay_Amount = trailMatch.Groups["Total_Patient_Pay_Amount"].Value.Trim();
                        _context.SaveChanges();
                    }
        ...
        private Regex oRxParse_Header 
        {
            get 
            {
                return new Regex(@"PA(?<Version_Number>.{2})(?<Sender_ID>.{24})(?<Batch_Number>.{7})(?<Creation_Date>.{8})(?<Creation_Time>.{4})(?<Receiver_ID>.{24})(?<ReportStartDate>.{8})(?<ReportEndDate>.{8})(?<FileType>.{1})(?<Transmission_Action>.{1})(?<Submission_Number>.{2})");
            }
        }
        private Regex oRxParse_Detail
        {
            get 
            {
        ...
        private Regex oRxParse_Compound1
        {
            get 
            {
                StringBuilder sb = new StringBuilder();
        ...
        private Regex oRxParse_Compound2
        {
            get 
            {
                StringBuilder sb = new StringBuilder();
        ...
        private Regex oRxParse_Trailer 
        {
            get 
            {
                StringBuilder sb = new StringBuilder();
